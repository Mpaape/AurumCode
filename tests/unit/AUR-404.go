...
const docsToolSchemaPath404 = ".board/schemas/docs-tool-offline-profile.schema.json"
...
const polyglotSchemaPath404 = ".board/schemas/polyglot-toolchain-profile.schema.json"
const polyglotLockPath404 = ".board/locks/oci/polyglot-toolchain-v1.lock.json"
...
var registryKeys404 = []string{"bootstrap-readonly-v1", "docs-tool-offline-v1", "fake-provider-v1", "fake-scm-offline-v1", "go-git-offline-v1", "go-unit-offline-v1", "oci-conformance-v1", "parser-worker-v1", "polyglot-toolchain-v1", "registry-v1", "sqlite-offline-v1"}
...
// ValidateRegistryAUR404 resolves the canonical registry and admits exactly the eleven
...
func ValidateRegistryAUR404(root string) (string, string) {
	...
	for _, x := range r.Profiles {
		...
		case "parser-worker-v1":
			// Owned by AUR-406, held to the same neighbour rule as AUR-403's and
		...
		case "docs-tool-offline-v1":
			...
			if x.Schema != docsToolSchemaPath404 || x.Lock != docsToolLockPath404 {
				return "", "unsafe-plan"
			}
			if !digest404.MatchString(x.SchemaDigest) || !digest404.MatchString(x.LockDigest) || !digest404.MatchString(x.ImageSetDigest) {
				return "", "digest-invalid"
		...
		case "polyglot-toolchain-v1":
			...
			if x.Schema != polyglotSchemaPath404 || x.Lock != polyglotLockPath404 {
				return "", "unsafe-plan"
			}
			if !digest404.MatchString(x.SchemaDigest) || !digest404.MatchString(x.LockDigest) || !digest404.MatchString(x.ImageSetDigest) {
				return "", "digest-invalid"
