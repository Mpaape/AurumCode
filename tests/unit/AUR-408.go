...
const polyglotSchemaPath408 = ".board/schemas/polyglot-toolchain-profile.schema.json"
const polyglotLockPath408 = ".board/locks/oci/polyglot-toolchain-v1.lock.json"
...
var registryKeys408 = []string{"bootstrap-readonly-v1", "docs-tool-offline-v1", "fake-provider-v1", "fake-scm-offline-v1", "go-git-offline-v1", "go-unit-offline-v1", "oci-conformance-v1", "parser-worker-v1", "polyglot-toolchain-v1", "registry-v1", "sqlite-offline-v1"}
...
// ValidateRegistryAUR408 resolves the canonical registry and admits exactly the eleven
...
func ValidateRegistryAUR408(root string) (string, string) {
	...
	for _, x := range r.Profiles {
		...
		case "polyglot-toolchain-v1":
			...
			if x.Schema != polyglotSchemaPath408 || x.Lock != polyglotLockPath408 {
				return "", "unsafe-plan"
			}
			if !digest408.MatchString(x.SchemaDigest) || !digest408.MatchString(x.LockDigest) || !digest408.MatchString(x.ImageSetDigest) {
				return "", "digest-invalid"
