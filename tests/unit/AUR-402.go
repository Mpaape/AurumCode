...
func ValidateRegistryAUR402(root string, document []byte) (string, string) {
	...
	for i, e := range r.Profiles {
		...
		if e.Key != "bootstrap-readonly-v1" && e.Key != "registry-v1" && e.Key != "go-unit-offline-v1" && e.Key != "go-git-offline-v1" && e.Key != "fake-provider-v1" && e.Key != "parser-worker-v1" && e.Key != "sqlite-offline-v1" && e.Key != "docs-tool-offline-v1" && e.Key != "fake-scm-offline-v1" && e.Key != "oci-conformance-v1" && e.Key != "polyglot-toolchain-v1" {
			return "", "profile-unregistered"
		}
		// Registered by AUR-411, under the same neighbour rule as the conformance profile
		// below: the polyglot profile's own documents are validated by that card's loader,
		// so this loader checks only the declared paths and digest shape and never reads
		// a file outside its own materialized allowlist.
		if e.Key == "polyglot-toolchain-v1" {
			if e.Schema != ".board/schemas/polyglot-toolchain-profile.schema.json" || e.Lock != ".board/locks/oci/polyglot-toolchain-v1.lock.json" {
				return "", "unsafe-plan"
			}
		...
		// Registered by AUR-410, under the same neighbour rule as the fake SCM below:
		// the conformance profile's own documents are validated by that card's loader,
		// so this loader checks only the declared paths and digest shape and never reads
		// a file outside its own materialized allowlist.
		if e.Key == "oci-conformance-v1" {
			if e.Schema != ".board/schemas/oci-conformance-profile.schema.json" || e.Lock != ".board/locks/oci/oci-conformance-v1.lock.json" {
				return "", "unsafe-plan"
			}
			if !digestAUR402.MatchString(e.SchemaDigest) || !digestAUR402.MatchString(e.LockDigest) || !digestAUR402.MatchString(e.ImageSetDigest) {
				return "", "digest-invalid"
