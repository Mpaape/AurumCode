...
go_case(){ local label="$1" pkg="$2" fn="$3" src="$4" out rc; local root="$run_dir/root-$label"; common "$root"; copy "$root" "$src"; printf 'package %s\nimport "testing"\nfunc TestAUR403Bridge(t *testing.T){ %s(t) }\n' "${pkg##*/}" "$fn" >"$root/$pkg/aur403_bridge_test.go"; set +e; out="$(cd "$root" && AURUMCODE_ROOT="$root" GOPROXY=off GOSUMDB=off GONOSUMDB='*' GOTOOLCHAIN=local GOMAXPROCS=1 GOCACHE="$run_dir/cache" GOTMPDIR="$run_dir/gotmp" go test -v -mod=readonly -p 1 "./$pkg" -run '^TestAUR403Bridge$' -count=1 2>&1)"; rc=$?; set -e; printf '%s\n' "$out"; ((rc==0)) || fail "selector:$label:exit:$rc"; grep -Eq '(^|[[:space:]])ok[[:space:]]' <<<"$out" || fail "selector:$label:zero-tests"; rm -rf -- "$root"; }
...
mutation_case(){ local mutation="$1" out rc; local expected="${2:-$mutation}"; local root="$run_dir/root-mut-$mutation"; common "$root"; copy "$root" tests/unit/AUR-403.go; printf 'package unit\nimport "testing"\nfunc TestAUR403Bridge(t *testing.T){ TestAUR403(t) }\n' >"$root/tests/unit/aur403_bridge_test.go"; set +e; out="$(cd "$root" && AURUMCODE_ROOT="$root" AURUM_A403_MUTATION="$mutation" GOPROXY=off GOSUMDB=off GONOSUMDB='*' GOTOOLCHAIN=local GOMAXPROCS=1 GOCACHE="$run_dir/cache" GOTMPDIR="$run_dir/gotmp" go test -v -mod=readonly -p 1 ./tests/unit -run '^TestAUR403Bridge$' -count=1 2>&1)"; rc=$?; set -e; printf '%s\n' "$out"; ((rc==0)) || fail "mutation:$mutation:exit:$rc"; grep -Fq "mutation=$mutation code=$expected" <<<"$out" || fail "mutation:$mutation:not-rejected"; rm -rf -- "$root"; }
run_all(){ go_case unit tests/unit TestAUR403 tests/unit/AUR-403.go; go_case contract tests/contracts ContractAUR403 tests/contracts/AUR-403.go; go_case integration tests/integration IntegrationAUR403 tests/integration/AUR-403.go; e2e_case; aur402_case; mutation_case duplicate-profile; mutation_case mutable-input; mutation_case unsafe-plan; mutation_case missing-required schema-invalid; mutation_case split-command unsafe-plan; printf '{"card":"%s","scenario":"%s","result":"valid","engine_invocations":0}\n' "$card" "$scenario"; }
case "$selector" in
 AC-001) run_all;;
