...
go_case(){ local label="$1" pkg="$2" fn="$3" src="$4" out rc; local root="$run_dir/root-$label"; common "$root"; copy "$root" "$src"; printf 'package %s\nimport "testing"\nfunc TestAUR403Bridge(t *testing.T){ %s(t) }\n' "${pkg##*/}" "$fn" >"$root/$pkg/aur403_bridge_test.go"; set +e; out="$(cd "$root" && AURUMCODE_ROOT="$root" GOPROXY=off GOSUMDB=off GONOSUMDB='*' GOTOOLCHAIN=local GOMAXPROCS=1 GOCACHE="$run_dir/cache" GOTMPDIR="$run_dir/gotmp" go test -mod=readonly -p 1 "./$pkg" -run '^TestAUR403Bridge$' -count=1 2>&1)"; rc=$?; set -e; printf '%s\n' "$out"; ((rc==0)) || fail "selector:$label:exit:$rc"; grep -Eq '(^|[[:space:]])ok[[:space:]]' <<<"$out" || fail "selector:$label:zero-tests"; rm -rf -- "$root"; }
