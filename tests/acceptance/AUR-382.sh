...
readonly card='AUR-382'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR382|IntegrationAUR382|E2EAUR382) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/SETUP_GUIDE.md"
...
claims="$repo_root/tests/migration/legacy/public-docs/AUR-382/claims.tsv"
[[ -s "$claims" && ! -L "$claims" ]] || fail claim_without_disposition
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='SETUP_GUIDE.md' -v digest="$source_sha" '
