...
source_file="$repo_root/.gemini/settings.json"
...
manifest="$repo_root/tests/characterization/legacy/agent-settings/AUR-387/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail settings_parse_inconclusive
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='.gemini/settings.json' -v digest="$source_sha" '
