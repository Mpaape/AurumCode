...
source_file="$repo_root/.claude/settings.local.json"
...
manifest="$repo_root/tests/characterization/legacy/agent-settings/AUR-389/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail settings_parse_inconclusive
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='.claude/settings.local.json' -v digest="$source_sha" '
