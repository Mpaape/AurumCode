...
readonly card='AUR-365'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR365|IntegrationAUR365|E2EAUR365) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/scripts/action-entrypoint.sh"
...
manifest="$repo_root/tests/characterization/legacy/script-files/AUR-365/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail legacy_source_uncharacterized
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='scripts/action-entrypoint.sh' -v digest="$source_sha" '
