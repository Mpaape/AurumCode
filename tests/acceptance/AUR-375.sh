...
readonly card='AUR-375'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR375|IntegrationAUR375|E2EAUR375) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/run-docs-pipeline.bat"
...
manifest="$repo_root/tests/characterization/legacy/script-files/AUR-375/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail legacy_source_uncharacterized
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='run-docs-pipeline.bat' -v digest="$source_sha" '
