...
readonly card='AUR-372'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR372|IntegrationAUR372|E2EAUR372) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/scripts/setup-github-app.sh"
...
manifest="$repo_root/tests/characterization/legacy/script-files/AUR-372/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail legacy_source_uncharacterized
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='scripts/setup-github-app.sh' -v digest="$source_sha" '
