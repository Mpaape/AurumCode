...
source_file="$repo_root/docs/getting-started.md"
...
claims="$repo_root/tests/characterization/legacy/guide-claims/AUR-384/claims.tsv"
[[ -s "$claims" && ! -L "$claims" ]] || fail guide_claim_unmapped
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='docs/getting-started.md' -v digest="$source_sha" '
