...
readonly card='AUR-380'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR380|IntegrationAUR380|E2EAUR380) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/PAGES_SETUP.md"
...
claims="$repo_root/tests/migration/legacy/public-docs/AUR-380/claims.tsv"
[[ -s "$claims" && ! -L "$claims" ]] || fail claim_without_disposition
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='PAGES_SETUP.md' -v digest="$source_sha" '
