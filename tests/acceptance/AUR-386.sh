...
readonly card='AUR-386'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR408|IntegrationAUR408|E2EAUR408) ;;
...
manifest="$repo_root/tests/characterization/legacy/agent-settings/AUR-386/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail settings_parse_inconclusive
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='.cursor/mcp.json' -v digest="$source_sha" '
