...
readonly card='AUR-370'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR392|IntegrationAUR392|E2EAUR392) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/scripts/generate-enhanced-docs.sh"
...
awk -F '\t' -v target='scripts/generate-enhanced-docs.sh' -v digest="$source_sha" '
