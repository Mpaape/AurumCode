...
card_file="$repo_root/.board/cards/backlog/AUR-203.md"
[[ -f "$card_file" && ! -L "$card_file" ]] || fail card_spec_missing
expected_csv="$(sed -n 's/^depends_on: \[\(.*\)\]$/\1/p' "$card_file" | tr -d ' ')"
[[ -n "$expected_csv" ]] || fail dependency_set_missing
suite="$repo_root/tests/acceptance/gates/AUR-203/suite.tsv"
[[ -s "$suite" && ! -L "$suite" ]] || fail required_manifest_missing
