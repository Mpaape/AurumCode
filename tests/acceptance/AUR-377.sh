...
readonly card='AUR-377'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR377|IntegrationAUR377|E2EAUR377) ;;
...
claims="$repo_root/tests/migration/legacy/public-docs/AUR-377/claims.tsv"
[[ -s "$claims" && ! -L "$claims" ]] || fail claim_without_disposition
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='README.md' -v digest="$source_sha" '
