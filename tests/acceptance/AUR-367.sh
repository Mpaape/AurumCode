...
readonly card='AUR-367'
selector="${1:-AC-001}"
case "$selector" in
  AC-001|ContractAUR367|IntegrationAUR367|E2EAUR367) ;;
  *) printf '%s/AC-001/unknown-selector\n' "$card" >&2; exit 64 ;;
esac

repo_root="$(CDPATH= cd -- "$(dirname -- "$0")/../.." && pwd -P)"
source_file="$repo_root/scripts/bulk-enable-repos.sh"
...
manifest="$repo_root/tests/characterization/legacy/script-files/AUR-367/manifest.tsv"
[[ -s "$manifest" && ! -L "$manifest" ]] || fail legacy_source_uncharacterized
source_sha="$(sha256sum "$source_file" | awk '{print $1}')"
awk -F '\t' -v target='scripts/bulk-enable-repos.sh' -v digest="$source_sha" '
