...
func ContractAUR404(t *testing.T) {
	...
	// (fake-scm-offline-v1, ninth key), then by AUR-410 (oci-conformance-v1, tenth key),
	// then by AUR-411 (polyglot-toolchain-v1, eleventh key). The arity assertion is kept
	// exact, not relaxed to a minimum: every rejection this check made before still fails,
	// only the registered count moved with the registry.
	if !ok || len(entries) != 11 {
		t.Fatalf("registry does not publish exactly eleven profiles: %v", registry["profiles"])
	}
	found := false
