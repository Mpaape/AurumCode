...
type contractFixtureAUR006 struct {
...
// ContractAUR006 is the contract-level entrypoint named by the card.
func ContractAUR006(t *testing.T) {
	t.Helper()
	root := os.Getenv("AURUMCODE_ROOT")
	if root == "" {
		t.Fatal("AURUMCODE_ROOT is required when running the standalone contract")
	}
	fixtureBytes, err := os.ReadFile(filepath.Join(root, "tests/specs/AUR-006/cases.yaml"))
	if err != nil {
		t.Fatalf("AUR-006 fixture unreadable: %v", err)
	}
	var fixture contractFixtureAUR006
