...
| PR-REV-006 | Offer built-in, versioned reviewer profiles (e.g. SOLID reviewer, security reviewer, performance reviewer) selectable per repository and per review, that set review emphasis and enabled rule families. | Every built-in profile is deterministic and versioned; switching profile changes only emphasis and enabled rules, never severity gates, redaction, or cost caps; an unknown profile fails closed. | contract, golden, security, E2E |
...
| PR-DOC-005 | Publish a validated tutorial for every user-facing use case, executable end to end from a clean clone. | Every use case has a tutorial whose exact commands run in CI and whose observed output matches the committed expected output; a tutorial without a passing execution gate is not published. | docs, E2E, golden |
...
| PR-REL-002 | Generate a deterministic changelog entry and the next semantic version for a reviewed change from its commit messages (Conventional Commits), with optional publication in the review and CI. | The same commit range yields the same entry and version with no LLM and no network; commit text never authorizes a transition (tag/release) and is never executed. | unit, fixture, contract, secret canary |
