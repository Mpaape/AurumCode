...
1. Leia o card inteiro e confirme `paths`, `read_paths`, `forbidden_paths`,
   `validation`, `container_profile`, `profile_owner` e o comando de
   acceptance. Antes de mover para `ready`, confirme que o dono do profile é
   upstream no DAG e que `read_paths: []` está explícito quando vazio. Todo
   card que usa Go declara `go.mod` e `go.sum` em `read_paths`; nao invente uma
   regra de validacao depois do commit.
