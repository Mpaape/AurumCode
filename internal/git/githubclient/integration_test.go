...
// Restored from commit c12d7ab. Adaptations, matching client_test.go: the
// review-flow server now also answers GET /repos/owner/repo with a
// permissions fixture (the restored client verifies write permission once,
// caching the verdict, before it publishes), so the expected request count
// for the full flow went from 5 to 6; and a read-only counterpart proves the
// same flow refuses to publish.
...
func TestIntegration_PullRequestReviewFlow(t *testing.T) {
	...
	// 3. Verify write permission (once, cached)
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/repos/owner/repo":
			...
			w.Write([]byte(`{"permissions":{"admin":false,"maintain":false,"push":true,"pull":true}}`))
...
+package main
 func main() {}`))
	...
	// Verify all requests were made. The permission check runs once for the
	// first publish and its verdict is cached for the rest.
	expectedRequests := 6 // 1 diff + 1 files + 1 permission + 2 comments + 1 status
	if requestCount != expectedRequests {
		t.Errorf("expected %d requests, got %d", expectedRequests, requestCount)
	}
}

func TestIntegration_ReadOnlyTokenRefusesToPublish(t *testing.T) {
	// The same review flow with a read-only token must still read the PR
	// but refuse to publish anything: no POST may reach the server.
...
+package main
 func main() {}`))
	...
	// Reading is allowed with a read-only token.
	diff, err := client.GetPullRequestDiff(ctx, "owner", "repo", prNumber)
	if err != nil {
		t.Fatalf("GetPullRequestDiff failed: %v", err)
	}
	...
	// Publishing is refused before any POST is sent.
	comment := ReviewComment{Body: "finding", CommitID: "sha", Path: "main.go", Line: 1}
	if err := client.PostReviewComment(ctx, "owner", "repo", prNumber, comment, "k"); !errors.Is(err, ErrNoWritePermission) {
		t.Fatalf("expected ErrNoWritePermission, got: %v", err)
	}
	if err := client.PostIssueComment(ctx, "owner", "repo", prNumber, "summary"); !errors.Is(err, ErrNoWritePermission) {
		t.Fatalf("expected ErrNoWritePermission, got: %v", err)
	}
